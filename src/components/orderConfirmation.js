...
const Confirmation = (props) => {
	...
	const handleSubmit = (e) => {
		...
		props.form.validateFieldsAndScroll((err, values) => {
			...
			if (!err) {
				AxiosAuth()
					.put(`https://whispering-dawn-20611.herokuapp.com/api/store/cart/${cartId}/approve`, payload)
					.then((res) => {
								...
								<List.Item>
									{!cartContents.finalLock ? (
										<div className="controls">
											<div onClick={() => subtract(item._id)} className="clicks">
												-
											</div>
			...
			<div className="lower">
				<Form onSubmit={!cartContents.finalLock ? handleSubmit : confirmPayment}>
					...
					<Form.Item label="Collection/Delivery Date">
						...
						})(<DatePicker disabled={cartContents.finalLock} disabledDate={disabledDate} />)}
					</Form.Item>
