...
const AddPhoto = ({ productData, setProductData, errorState }) => {
  ...
  const uploadImage = (e) => {
    ...
    axios
      ...
      .then((res) => {
        setProductData({
          ...productData,
          ['images']: [...productData.images, res.data.secure_url],
        });
      });
  ...
  return (
    <div className='photoContainer' data-testid='addPhotoWrapper'>
      <div className='PhotoHeaderContainer' data-testid='photoHeaderWrapper'>
        <h3>Photos</h3>
      ...
      <h4 className='validationMessage'>
        {imageCount === 3 ? 'Three photo limit met' : ''}
      </h4>
      {loading
        ? 'Loading...'
        : productData.images.map((cv) => {
            return (
              <div className='singleProductImage' key={Math.random()}>
                <img className='productImage' src={cv} alt='' />
                <img
                  className='trashIcon'
                  src={trashIcon}
                  alt=''
      ...
      <div className={errorState === 'images' ? 'error' : 'hideError'}>
