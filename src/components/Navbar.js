...
const Navbar = () => {
	...
	const url = `${window.location.origin.toString()}/store/${user &&
		user.storeName &&
		user.storeName.toLowerCase().split(' ').join('-')}-${user && user._id}`;
	...
	return (
		<div className="nav-bar">
			<Row justify="space-around" gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} className="link-container">
				<Row justify="space-between" align="middle" gutter={[ 8, 8 ]} className="nav-links">
					<Col className="gutter-row" span={4}>
						<img src={NoLogo} alt="Pure Retail Logo" className="pure-retail-logo" />
					</Col>
				...
				<Row justify="space-around" align="middle" gutter={[ 8, 8 ]} className="btn-container">
					<Col>
