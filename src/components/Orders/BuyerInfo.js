...
const BuyerInfo = ({ fullOrder, setOrderCanceled, orderId }) => {
  ...
  function nextStatus(arg) {
    ...
    if (currentStatus === 'Prepaired') {
      ...
      return axiosWithAuth()
        .put(
          `https://shopping-cart-be.herokuapp.com/api/store/order/${orderId}`,
          payload
        )
        .then((res) => {
  ...
  return (
    <div className='buyerInfoContainer' data-testid='buyerInfoWrapper'>
      <div className='buyerInfoCard'>
        <section className='topSection'>
          ...
          <div className={`orderStatus ${currentStatus}`}>
            {' '}
            {currentStatus}{' '}
          </div>
        </section>
        ...
        <section className='bottomSection'>
          ...
          <div className='startDataContainer'>
            ...
            <h3>
              ...
              {completedTime === '-'
                ? '-'
                : moment(completedTime).format('MMM DD YYYY')}
            </h3>
          </div>
        </section>
      </div>
      <div className='actionContainer'>
        {currentStatus === 'Complete' || currentStatus === 'Canceled' ? (
          ...
          <div>
            <button
              className={
                buttonStatus === 'Complete Order' ? 'completeBTN' : 'ReadyBTN'
              }
              onClick={() => {
