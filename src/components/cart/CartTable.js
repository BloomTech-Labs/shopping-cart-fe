...
const CartTable = ({ cartContents, totalPrice, store }) => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://shopping-cart-be.herokuapp.com/api/auth/pk/${getStoreID[1]}`
      )
      .then((res) => {
  ...
  const submitHandler = async (event) => {
    ...
    axios
      .post(
        "https://shopping-cart-be.herokuapp.com/api/stripe/payment/create-payment-intent",
        paymentPayload
      )
      .then((res) => {
        ...
        axios
          .post(
            `https://shopping-cart-be.herokuapp.com/api/store/${getStoreID[1]}/order`,
            orderPayload
          )
          .then((res) => {
      ...
      {cartContents.length > 0 ? (
        <div className="totalPrice">
          ...
          >
            ...
            <h3>
              All transactions are secured through Stripe! Once payment is
              confirmed you will be directed to a confirmation screen
            </h3>
