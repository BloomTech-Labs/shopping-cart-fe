			...
			<div className="store-top review-top">
				<div className="review-text">
					...
					<div id="rev">
						...
						<p id="grey">
							This is your chance to check your order before going to payments. Make it count.
						</p>
					</div>
					<div>
						<p>Got a question about your order? You can ask the seller on Whatsapp by clicking here.</p>
					</div>
				</div>
				<div className="content">
						...
						renderItem={(item) => (
							<List.Item>
								<div className="controls">
									<div onClick={() => decrement(item.productId)} className="clicks">
										-
									</div>
								...
								<List.Item.Meta title={item.name} description={`${sign}${item.price}`} />
								<div onClick={() => removeItem({ _id: item.productId })} className="cancel">
									X
								</div>
							</List.Item>
						)}
					...
					{cartContents.length > 0 ? (
						<div className="button-body">
							<div onClick={showModal} style={{ backgroundColor: '#0B3954' }} className="button">
								Save for later
							</div>
							<AddEmail
								ref={saveFormRef}
