...
export const getCurrentUser = () => (dispatch) => {
	AxiosAuth()
		...
		.then((res) => {
			...
			AxiosAuth()
				.get(`https://whispering-dawn-20611.herokuapp.com/api/store/${res.data._id}/products`)
				.then((res) => {
...
export const getProducts = (sellerId, signal) => (dispatch) => {
	axios
		.get(`https://whispering-dawn-20611.herokuapp.com/api/store/${sellerId}/products`)
		.then((res) => {
			const inventory = res.data;
			dispatch({ type: types.GET_INVENTORY, payload: inventory });
		})
